...
ToolPrompt submissionPre-tool usePost-tool useFile readAgent responseAgent thoughtCursor✓✓✓✓✓✓Claude Code✓✓✓—✓—Claude Cowork✓✓✓—✓—VS Code✓✓✓———GitHub Copilot✓✓✓———OpenAI Codex✓✓✓—✓—Kiro CLI✓✓✓—✓—Devin CLI✓✓✓—✓—Devin Desktop✓✓✓✓✓—Google Antigravity—✓————Snowflake Cortex Code✓✓✓———
...
ToolPrompt submission: BlockPre-tool use: BlockPre-tool use: Modify inputPost-tool use: BlockPost-tool use: Modify outputRequire Approval (pre-tool use)Cursor✓✓✓—MCP tools onlyBlocks insteadClaude Code✓✓✓✓✓Native askClaude Cowork✓✓✓✓✓Blocks insteadVS Code✓✓✓✓—Native askGitHub Copilot—✓✓—✓Native askOpenAI Codex✓✓✓✓—Blocks insteadKiro CLI—✓ †———Blocks insteadDevin CLI✓✓✓——Blocks insteadDevin Desktop✓✓———Blocks insteadGoogle Antigravity—✓———Native askSnowflake Cortex Code✓✓✓——Blocks instead
...
**Require Approval (pre-tool use)** — *Native ask* means the integration's pre-tool-use hook exposes an ask/confirm response, so the tool call is paused and put to the developer. *Blocks instead* means it does not, so Require Approval denies the tool call rather than pausing it. This column is independent of the `†` marker above.

The matrix above covers block and modify support. **Require Approval** — a policy action that holds an operation for a person to approve rather than blocking or modifying it — is configured on the AI Gateway; see [Policy Actions](/_docs/applications/ai_gateway#policy-actions) for what it does and how a held operation resolves. Whether a specific coding-agent integration can ask or block on Require Approval is shown in the **Require Approval (pre-tool use)** column above.

Two different mechanisms are easy to confuse here. The AI Gateway's turn-based **hold** — where the operation is parked and the user resolves it by replying in the conversation, as described in [Policy Actions](/_docs/applications/ai_gateway#policy-actions) — is not available on any coding-agent integration, because the hooks provide no channel to resume a parked operation later. What some integrations do provide is a native **ask**: the pre-tool-use hook can put the tool call to the developer and wait for an inline answer. Only pre-tool use can ask, and only on the integrations marked *Native ask* above — post-tool use does not provide a resumable ask channel on any integration; its behavior may be blocking, modifying, or observational depending on the integration, and prompt submission, file read, agent response, and agent thought cannot ask either. Where an event cannot ask, Require Approval falls back to blocking: the operation is denied rather than put to the developer. Pre-tool use is therefore the event to target when you want a prompt rather than a block.
...
- **Claude Cowork — the ask response is ignored.** Cowork's pre-tool-use hook accepts an ask response but does not act on it: the tool call proceeds as if nothing were asked. Because an ignored ask would fail open, Require Approval blocks on Cowork instead of asking. Block and modify actions are unaffected and work on all tool types.
- **Snowflake Cortex Code — post-tool use is detection-only.** Cortex Code's post-tool-use hook supports neither a block response nor an output rewrite, so post-tool use policies record the event for audit without enforcing. Use pre-tool use policies, which support both blocking and input modification, to stop or rewrite specific tool calls.
...
- **Prompt submission** is blocking for Cursor, Claude Code, Claude Cowork, VS Code, OpenAI Codex, Devin CLI, Devin Desktop, and Snowflake Cortex Code — it can prevent the prompt from being processed. For GitHub Copilot and Kiro CLI, prompt submission is observational only (Kiro surfaces the policy reason as a warning). Google Antigravity does not intercept prompt submission.
- **Pre-tool use** is blocking across all integrations — it can prevent the tool from executing. It can also modify the tool input on Cursor, Claude Code, Claude Cowork, VS Code, GitHub Copilot, OpenAI Codex, Devin CLI, and Snowflake Cortex Code.
- **File read** is blocking for Cursor and Devin Desktop — it can prevent the agent from reading a file.
- **Post-tool use** varies by integration. Because it runs after the tool has completed, a post-tool-use block stops the agent from continuing with the result but cannot undo the tool's side effects. For Claude Code and Claude Cowork it can block, or modify tool output for all tools (surfacing the policy reason alongside the rewrite as additional context). For Cursor it can add context or modify MCP tool output but cannot block. For VS Code and OpenAI Codex it can block only. For GitHub Copilot it can modify the tool result or add context but cannot block. For Kiro CLI, Devin CLI, Devin Desktop, and Snowflake Cortex Code it is observational.
...
- **Pre-tool use** can modify tool input on Cursor, Claude Code, Claude Cowork, VS Code, GitHub Copilot, OpenAI Codex, Devin CLI, and Snowflake Cortex Code. Kiro CLI, Devin Desktop, and Google Antigravity provide no rewrite channel, so pre-tool use can block but not modify on those integrations.
...
- Claude Cowork can modify tool output for **all tool types** through the `updatedToolOutput` response field, in the same way as Claude Code.
...
- VS Code, OpenAI Codex, Kiro CLI, Devin CLI, Devin Desktop, and Snowflake Cortex Code do **not** support modifying tool output. If a guardrail needs to address built-in tool output, use the pre-tool use hook or rely on blocking.

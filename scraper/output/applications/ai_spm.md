...
Use the AI Security Posture Management (SPM) application to set policies for security posture and track adherence to the policies, uncover and remediate vulnerabilities, misconfigurations and other security issues, and maintain secure environments where you develop AI.

The AI-SPM application inspects AI resources that are a part of your [discovered AI systems](/_docs/docs/applications/ai_inventory). Once resources are added to inventory the AI-SPM application starts evaluating the resources for issues as per your posture policies. The SPM evaluators run nightly as part of the system ETL.

Posture is assessed on many dimensions - vulnerabilities, misconfigurations, notebook scanners, model/LLM scans/pentests and more. Posture is assessed once a day based on all resources that are managed within the system (e.g. that are discovered by the system as part of the AI Inventory application).

## Dashboard[​](#dashboard)
View vulnerabilities, misconfigurations, pentest findings and exposed resources for the selected projects or organizations and drill down into each finding directly from the dashboard. The Security Posture Compliance Heatmap depicts your posture as a heatmap wgere the x-axis are the policies and the y-axis are the projects you have selected.

## Policies[​](#policies)
Configure a policy or select from existing policies and then track your posture based on the policy. You can set a policy for cloud scans and a policy for notebook scans. Cloud posture management allows you to select from a large number of posture checks. Notebook scanners allow you to scan for PII in notebooks, secrets stored in notebooks and notebook vulnerabilities.

Click on Open to view adherence with the policy and drift over time. All issues are listed by category and you can drill into the specific policy details for each item.
...
- Select your LLM Endpoint – Choose the target resource from your inventory. Note that this can be a custom LLM endpoint that is really your application API.
- Describe the System – Provide a brief system description that will appear in the report.
- Select the Model (if applicable) – If the endpoint supports multiple models, choose the one to test. If this is a custom LLM endpoint (an application) no selection is appropriate and the drop down will be disabled.
...
- Add information about the AI system. The system uses this details to craft specialized tests. The more details you provide the better the customization will be.
- Enable active gateway policies - when you check this button the pentests runs through the guardrails system. This allows you to test how effective the guardrails are (and you can also compare with/without guardrails).
...
Note related to LLM deployed as Watson.x endpoints: You can pentest /chat LLM endpoints based on Mistral and Llama. Watson.x provides an abstraction for chat endpoints but it is not implemented yet for Granite models, and you will get an error of the form:
...
- You'll provide a name and description (e.g., "Refusal Category that checks if the model refuses to answer") and select a severity level (Low, Medium, High).
...
When creating a custom category, you choose an Evaluation Mechanism from a dropdown. These define the "Evaluation Criteria" for how the LLM's response will be assessed. You can hover over the information icon for each option in the UI to see its description:

- Varonis Default: A general-purpose test that checks the LLM's response for any harmful, malicious, or inappropriate content. Any unsafe output results in a failure. No evaluation criteria is required.
- Refusal as Pass: The test passes only if the chatbot refuses to answer the input request, indicating proper safety controls. No evaluation criteria is required.
- Refusal as Fail: The test passes only if the chatbot does not refuse the input request, verifying that it is willing to respond when appropriate. No evaluation criteria is required.
...
The Custom LLM Endpoint integration allows you to register conversational (chatbot) APIs for penetration testing and proxying through the Gateway.

You define a custom LLM endpoint in AI Inventory -&gt; Technologies -&gt; Add New -&gt; Add New Resources Manually -&gt; LLM Endpoint. For Provider select Custom LLM Endpoint.

There are two ways to register the way that the target system expects the connection to be:
...
Consult your account manager with specific Application/API interaction pattern.
...
- Chat-like interface: Your API must accept free-form text (prompt) as input and return free-form text (completion) as output, like a typical chatbot. Non-chat (conversational) interfaces are not supported.
...
- Static URL: The API endpoint URL must be static and not change over time nor require dynamic path parameters.
...
- Once you choose Custom LLM Endpoint as the provider, you will see a set of fields as shown in the image below. An explanation of each fiied follows after the image.
...
Choose how we should parse the response:
...
The first step is to register an DSL Configuration. The [LLM Endpoint DSL Specification](/_docs/docs/applications/llm_dsl_specification) allows for inputs to be injected,
which are often used or resource identifiers or secrets. Later, you will be able to associate LLM Endpoints with a DSL Configuration.

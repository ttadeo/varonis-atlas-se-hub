...
## How MCP Security works[​](#how-mcp-security-works)
...
## Shadow MCP workflow[​](#shadow-mcp-workflow)
...
## Manage credentials for an MCP Server[​](#manage-credentials-for-an-mcp-server)
The authentication configured when you register an MCP Server — the connection-level credential used for MCP Inspector discovery — remains the server's default credential. After registration, you can add additional named credentials to the server, making it possible to connect to the same MCP Server with different identities or authentication contexts. These credentials can be shared across your tenant or scoped to specific owner contacts, and they can be tested, rotated, and associated with the server without affecting the server's registration configuration.

The **Manage Credentials** drawer on the MCP Server detail page in the Catalog provides access to the server's credential set. This surface is separate from the registration-time authentication options and is used to manage credentials after the server exists in inventory.

## Multiple named credentials[​](#multiple-named-credentials)
A registered MCP Server can have one or more additional credentials beyond the registration credential. Each credential has a display name, an authentication mechanism (API Key or OAuth), and a sharing mode. The server's default credential remains the credential configured at registration; the additional credentials are available for selection in other workflows, such as when creating Gateway Access Tokens.

Credentials can be associated with one or more MCP Servers. The same credential can be reused across multiple servers that accept the same authentication scheme, reducing the need to manage duplicate secrets.

## Credential authentication mechanisms[​](#credential-authentication-mechanisms)
Each credential is backed by one of two authentication mechanisms:

- **API Key** — A static bearer token that Atlas injects as an `Authorization: Bearer &lt;token&gt;` header on each request. The token is stored encrypted and is not displayed back after it is saved.
- **OAuth** — An OAuth credential obtained through the OAuth consent flow. OAuth credentials are minted when you authorize access through the vendor's consent screen. They are refreshed automatically and do not require manual rotation.

The authentication mechanism is chosen when the credential is created and cannot be changed. For servers that require different credential types, create separate credentials with the appropriate mechanism for each.

## Shared and per-user credentials[​](#shared-and-per-user-credentials)
Credentials can be **Shared** or **Per user**. Shared credentials are available to any user in your tenant who has permission to manage MCP configurations. Per-user credentials are associated with a specific owner contact email. The sharing mode is set when the credential is created and determines how the credential is surfaced in other parts of the platform.

When creating a per-user credential, an owner email is required. The email serves as a label and contact reference; it does not enforce runtime access control. Both shared and per-user credentials are tenant-scoped and can be associated with any MCP Server in the tenant.

## Testing and rotating credentials[​](#testing-and-rotating-credentials)
Credentials can be tested from the **Manage Credentials** drawer. Testing a credential initiates a connection attempt to the associated MCP Server using that credential's authentication mechanism. If the connection succeeds, the credential is marked as healthy. If it fails, the failure reason is displayed so the credential can be corrected or rotated.

Credentials that use API Key authentication can be rotated in place. Rotating a credential replaces its stored secret but preserves the credential's identity, so all server associations and Gateway Access Token bindings remain intact. OAuth credentials refresh automatically and do not require manual rotation.

## Virtual MCPs[​](#virtual-mcps)
...
## Gateway Access Tokens and per-token credentials[​](#gateway-access-tokens-and-per-token-credentials)
A Gateway Access Token is a bearer credential used by an MCP client to connect to the MCP gateway. Each token is tied to one or more Virtual MCPs, and it can include per-server credential bindings that determine which credential the gateway uses when connecting to upstream MCP Servers on behalf of the token.

Gateway Access Tokens are created from the Virtual MCP detail page in the **Gateway** tab. The token creation flow allows you to configure token metadata, select the Virtual MCPs the token applies to, and bind credentials on a per-server basis.

## Per-server credential bindings[​](#per-server-credential-bindings)
When creating or updating a Gateway Access Token, you can bind a specific credential to each MCP Server that is linked to the Virtual MCP. The binding specifies which credential the gateway injects when the token calls that server. Each token can have at most one credential bound per MCP Server.

If a server has no binding for a given token, requests from that token to that server are denied. There is no silent fallback to the server's default credential. This ensures that credential selection is always explicit and that tokens do not inadvertently gain access to servers they have not been configured for.

## Selecting credentials when creating a token[​](#selecting-credentials-when-creating-a-token)
The Gateway Access Token creation drawer presents a per-server credential selector for each MCP Server linked to the Virtual MCP's published version. For each server, you can choose from the credentials that are associated with that server, or you can leave the server unbound. Servers with a review status of **Rejected** are shown as blocked and cannot be bound.

Credentials available for selection include the server's registration credential and any additional credentials that have been associated with the server through the **Manage Credentials** drawer. Both shared credentials and per-user credentials appear in the selector, and you can bind any credential that is associated with the server.

If you do not bind a credential for a server, that server is not accessible through the token. To grant access, you must explicitly bind a credential.

## Updating bindings and credential health[​](#updating-bindings-and-credential-health)
After a token is created, its bindings can be updated without recreating the token. The bindings update flow allows you to change which credential is bound to each server, add new bindings for servers that were not initially configured, or remove bindings to revoke access to specific servers.

The Gateway Access Token detail view displays each bound credential's display name, connection status, and the time it was last tested. This health information reflects the most recent connection test for that credential and helps identify credentials that may need rotation or troubleshooting.

## Runtime enforcement with MCP Quarantine[​](#runtime-enforcement-with-mcp-quarantine)
...
[PreviousAI Runtime](/_docs/docs/applications/ai_gateway)[NextAI Investigation](/_docs/docs/applications/ai_monitor)- [MCP Security](#mcp-security)- [Why use MCP Security](#why-use-mcp-security)- [Key concepts](#key-concepts)- [MCP Server](#mcp-server)- [Tool](#tool)- [Shadow MCP](#shadow-mcp)- [Virtual MCP](#virtual-mcp)- [MCP Quarantine](#mcp-quarantine)- [How MCP Security works](#how-mcp-security-works)- [Discover MCP usage](#discover-mcp-usage)- [Review MCP Servers and tools](#review-mcp-servers-and-tools)- [Build allowlists with Virtual MCPs](#build-allowlists-with-virtual-mcps)- [Enforce the allowlist at runtime](#enforce-the-allowlist-at-runtime)- [Monitor posture and activity over time](#monitor-posture-and-activity-over-time)- [Catalog](#catalog)- [Marketplace and Internal MCP Servers](#marketplace-and-internal-mcp-servers)- [Shadow MCP workflow](#shadow-mcp-workflow)- [When a connection appears as Shadow MCP](#when-a-connection-appears-as-shadow-mcp)- [Resolve a Shadow MCP](#resolve-a-shadow-mcp)- [Register an MCP Server](#register-an-mcp-server)- [Connection type](#connection-type)[Transport types](#transport-types)- [Authentication types](#authentication-types)- [Setting up an IAM role for AWS IAM (SigV4)](#setting-up-an-iam-role-for-aws-iam-sigv4)- [Scope](#scope)- [What happens after registration](#what-happens-after-registration)- [Manage credentials for an MCP Server](#manage-credentials-for-an-mcp-server)- [Multiple named credentials](#multiple-named-credentials)- [Credential authentication mechanisms](#credential-authentication-mechanisms)- [Shared and per-user credentials](#shared-and-per-user-credentials)- [Testing and rotating credentials](#testing-and-rotating-credentials)- [Virtual MCPs](#virtual-mcps)- [Create a Virtual MCP](#create-a-virtual-mcp)- [VMCP scope](#vmcp-scope)- [Draft and published versions](#draft-and-published-versions)- [Add MCP Servers and tools to a VMCP](#add-mcp-servers-and-tools-to-a-vmcp)- [Effective tools](#effective-tools)- [Tool Update Handling](#tool-update-handling)- [Gateway Access Tokens and per-token credentials](#gateway-access-tokens-and-per-token-credentials)- [Per-server credential bindings](#per-server-credential-bindings)- [Selecting credentials when creating a token](#selecting-credentials-when-creating-a-token)- [Updating bindings and credential health](#updating-bindings-and-credential-health)- [Runtime enforcement with MCP Quarantine](#runtime-enforcement-with-mcp-quarantine)- [What MCP Quarantine checks](#what-mcp-quarantine-checks)- [Activity](#activity)- [Policies](#policies)- [Policy inheritance](#policy-inheritance)- [Supported MCP Policies](#supported-mcp-policies)- [Issues](#issues)- [Reports](#reports)- [Recommended workflow](#recommended-workflow)

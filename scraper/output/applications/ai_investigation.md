...
The **Assess** tab explains why an issue was created. It is part of the **AI Runtime → Issues** view (see [AI Runtime](/_docs/docs/applications/ai_gateway#issues)) and is always present there — if the policy that fired does not supply a trigger reason or evidence, the tab says so rather than being hidden. The AI Investigation per-event Assess view (below) presents the same reason-and-evidence model, described here, for policy violations other than Intent-Based Access Control (which renders on the event's Intent tab instead) — but omits the tab entirely for a violation with nothing to show.
...
The tab automatically chooses which combination to show based on what the runtime policy provided — reason and evidence, reason only, evidence only, or neither. Not every policy supplies one; see [Trigger reason support](#trigger-reason-support) below for which do.

#### Trigger reason support[​](#trigger-reason-support)
Not every policy explains itselfA trigger reason is written by the policy that fired, so what you see on the Assess tab depends on which policy created the issue.

- Policies that always supply a reason, evidence, or both are listed in the table below.
- Some policies supply a reason only when the underlying detector produces one, so the same policy may show a reason on one issue and nothing on another.
- Where a policy supplies neither, the AI Runtime Issues drawer shows **Reason unavailable** with a short explanatory line. This is expected, not an error — the issue itself and the rest of its detail are unaffected.
- AI Investigation behaves differently: a per-event violation with nothing to show is filtered out, and if no violation on the event has anything to show the Assess tab does not render at all.
- This list reflects the policies that supply trigger reasons today and is expected to grow.

PolicyWhat it showsPIIThe PII type detected and the matched textBanned SubstringsThe matched textRegex matchThe matched text; for pass-style rules, the pattern that was not metDetect LanguagesThe language detectedTopic policies (detect and prohibit)The topic detectedCode policies (leakage and injection)The programming language detected — never the code itselfJailbreakA short reason describing the detectionPrompt InjectionA short reason describing the detection
...
AI Investigation can assign a short, descriptive name to a session, helping you distinguish related sessions more easily. The name is generated by an AI model from up to the first five distinct, non-empty user prompts in the session. It does not use assistant responses, tool activity, or system instructions.

Names are created after a brief settling period rather than in real time. When available, the Sessions sidebar displays the name as the primary label and the session ID as a subtitle. The session detail page displays the name above the session ID.

If a descriptive name is unavailable, AI Investigation continues to display the session ID. Sessions whose prompt content is fully masked by a logging policy are not given descriptive names and keep their session ID. Session names are assigned in periodic batches, typically within minutes, so a name can appear after the session is created. To prevent an indefinite backlog, sessions that remain unnamed beyond the processing window retain their session ID as their permanent name.
...
**Assess tab (per-event).** Each event's detail includes an Assess tab when at least one policy violation, other than Intent-Based Access Control, has a displayable trigger reason or supporting evidence. The tab shows one collapsible section per such policy that fired, labeled **Triggered by [policy name]**. Each section presents the same trigger reason and supporting evidence described in [Assess tab: why an issue triggered](#assess-tab-why-an-issue-triggered). All sections are expanded by default so you can quickly review why each policy fired on that event. Intent-Based Access Control violations do not appear here — they render on the event's **Intent** tab instead.

Which policies supply a trigger reason is covered in [Trigger reason support](#trigger-reason-support). A violation from a policy that supplies neither a reason nor evidence is not shown here, and if no violation on the event has anything to show, the Assess tab does not appear on that event at all.

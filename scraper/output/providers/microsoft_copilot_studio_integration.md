...
url: https://prod.alltrue-be.com/_docs/docs/providers/copilot_studio
section: providers
---

# Microsoft Copilot Studio Integration

- [](/_docs/)- [Providers](/_docs/docs/providers)- MS Copilot StudioExport PDFOn this page# Microsoft Copilot Studio Integration
Microsoft [Copilot Studio](https://learn.microsoft.com/en-us/microsoft-copilot-studio/fundamentals-what-is-copilot-studio) lets you build custom AI agents that can invoke external tools, connectors, and APIs. Atlas integrates with Copilot Studio's [external threat detection](https://learn.microsoft.com/en-us/microsoft-copilot-studio/external-security-provider) feature to evaluate every tool invocation against your installed guardrail rules before the tool executes. If a rule blocks the action, Copilot Studio prevents the tool from running and reports the block reason to the user.
...
- **Discovery** — Atlas discovers your Copilot Studio agents, their components, tools, connectors, and authentication configurations from each Power Platform environment registered as its own cloud account.
- **Runtime Controls** — Every tool invocation in a Copilot Studio agent is sent to Atlas for policy evaluation before execution. Guardrail rules (PII detection, prompt injection protection, topic blocking, etc.) run on your data plane and return an Allow or Block decision.

## How It Works[​](#how-it-works)
When a Copilot Studio agent is about to invoke a tool:

- **Token exchange** — Copilot Studio obtains a Federated Identity Credential (FIC) token for the registered Entra application and sends it to Atlas webhook endpoint.
- **Token validation** — Atlas validates the Microsoft Entra ID token, extracting the tenant and agent identity.
...
- **Decision** — Atlas returns an **Allow** or **Block** response. If blocked, Copilot Studio prevents the tool from executing and surfaces the block reason.

Copilot Studio expects a response within one second. If Atlas does not respond in time, Copilot Studio allows the tool to execute by default (configurable — see [error behavior](#step-2-configure-threat-detection-in-power-platform)).
...
Discovery is scoped per registered Power Platform environment account — one account per Dataverse environment, registered through [Azure onboarding](/_docs/docs/providers/azure/onboarding). From each registered environment, Atlas discovers your Copilot Studio agents and their components: topics, knowledge sources, workflows, variables, triggers, translations, tools, connectors, and authentication configurations. See [Supported Resource Types](/_docs/docs/applications/ai_inventory/supported_resource_types) for the full per-resource-type breakdown.

An environment that is never registered, or is later deregistered, stops being monitored: its Copilot Studio resources leave the AI Inventory and its open issues auto-resolve until the environment is registered again.

Discovered agents and their dependencies appear in the **AI Inventory** and can be assigned to projects for governance.
...
The setup requires creating a Microsoft Entra application registration with a Federated Identity Credential (FIC). This allows Copilot Studio to authenticate with Atlas using token exchange.

Before you begin, verify that Atlas endpoint domain (or a parent domain) is registered as a verified custom domain in your Entra ID tenant. Navigate to **Azure Portal &gt; Microsoft Entra ID &gt; Custom domain names** to check. If the domain is not verified, the application registration will fail.

You have the following values ready (available from Atlas GUI — see below):

ValueDescription**Tenant ID**Your Azure AD tenant ID (GUID)**Endpoint**Atlas webhook URL. Format: `https://&lt;YOUR-DATA-PLANE&gt;/sdk/v1/webhooks/copilot-studio`**Data Plane ID**Identifier for your Atlas data plane
You can obtain these pre-filled from Atlas GUI through either of these paths:
...
- PowerShell Script (Recommended)- Manual Configuration (Azure Portal)Atlas GUI displays a ready-to-run PowerShell script with all values pre-filled. Copy it and run in PowerShell.
...
- **Endpoint**: your Atlas webhook URL (`https://&lt;YOUR-DATA-PLANE&gt;/sdk/v1/webhooks/copilot-studio`)
...
After the Entra application is registered, configure Copilot Studio to send tool invocations to Atlas. This setting is configured per Power Platform environment.
...
**Allow the agent to respond** (default) — if Atlas is unreachable or does not respond within one second, allow the tool invocation (fail open).
- **Block the query** — if Atlas is unreachable, block the tool invocation (fail closed).
...
When Copilot Studio calls Atlas for a tool invocation, the following data is included in the webhook payload:
...
Atlas evaluates both the **input** (what is being sent to the tool) and the **output** (what the tool would produce) concurrently. If either evaluation triggers a rule violation, the tool invocation is blocked.
...
Note: Unlike the [LiteLLM Proxy Integration](/_docs/providers/litellm_proxy_integration), Copilot Studio's threat detection webhook does not support content modification (e.g., PII redaction). If a rule would modify content, Atlas treats this as a block to prevent unmodified sensitive data from reaching the tool.
...
The request appears in Atlas **AI Runtime Protection** activity log.
...
- The block appears in Atlas activity log.

## Troubleshooting[​](#troubleshooting)
IssueResolution**Setup script fails with "Domain validation failed"**The endpoint domain must be verified in Microsoft Entra ID. Navigate to **Azure Portal &gt; Microsoft Entra ID &gt; Custom domain names** and add/verify the domain.**Setup script fails with 403 Forbidden**Ensure your account has Application Administrator, Cloud Application Administrator, or Global Administrator role in Entra ID.**Power Platform save fails with "problem connecting to the protection provider"**The endpoint is unreachable or timed out. Verify the endpoint URL is correct and accessible over HTTPS. Contact your account team if the data plane endpoint is not responding.**Power Platform save fails with "problem with the configuration"**Token acquisition failed. Check the Entra application configuration and Federated Identity Credentials. Select **Copy error info** in the Power Platform Admin Center for details.**Tool invocations are not appearing in Atlas**Check that threat detection is enabled for the correct Power Platform environment. Verify the Entra App ID matches the registered application.**All tool invocations are being blocked**Verify that guardrail rules are correctly configured for the endpoint. Check if the endpoint is marked as "sanctioned" in Atlas.**Authentication errors (401)**The Federated Identity Credential may be misconfigured. Re-run the setup script to verify or update the FIC configuration. The script detects existing applications and updates them if needed.**"No FIC configured on the app"**The Entra application has no Federated Identity Credential. Re-run the setup script to create the FIC.**"No matching federated identity record found for presented assertion issuer"**The FIC issuer does not match. Ensure the issuer is set to `https://login.microsoftonline.com/&lt;YOUR-TENANT-ID&gt;/v2.0`.[PreviousLiteLLM SDK Integration](/_docs/docs/providers/litellm_sdk_integration)[NextIBM Watsonx](/_docs/docs/providers/ibm_watsonx)- [How It Works](#how-it-works)- [Prerequisites](#prerequisites)- [Agent Discovery](#agent-discovery)- [Configuration](#configuration)[Step 1: Register the Entra Application](#step-1-register-the-entra-application)- [Step 2: Configure Threat Detection in Power Platform](#step-2-configure-threat-detection-in-power-platform)- [How Tool Invocations Are Evaluated](#how-tool-invocations-are-evaluated)[Decision Outcomes](#decision-outcomes)- [Verifying the Integration](#verifying-the-integration)- [Troubleshooting](#troubleshooting)

...
- **Cloud-discovery subscription link** — link your Azure subscriptions and run a discovery pipeline that inventories Azure OpenAI, Azure Databricks, Azure Machine Learning, storage, AI Search, Copilot Studio, and AI Foundry resources.
...
- **Copilot Studio discovery and runtime** — Copilot Studio agents are discovered from a linked subscription and protected at runtime.
...
## Link your Azure subscriptions for discovery[​](#link-your-azure-subscriptions-for-discovery)
Link your subscriptions from **AI Inventory → Configuration → Cloud Accounts** by selecting **Link New Account → Microsoft Azure Accounts**. Atlas supports two paths:

- **All subscriptions** — add all subscriptions under your Azure tenant in a single step. Required roles: **Application Administrator** and **Owner of the root management group**. Run the provided command in your `azure-cli` terminal to add all subscriptions.
- **Some subscriptions** — selectively add specific Azure subscriptions. Required roles: **Application Administrator** and **Owner of the subscriptions you want to add**. Enter the subscription IDs, click **Get Command**, then run the generated `azure-cli` command.

After running the command, use **Test Connection** to verify the link before you commit. Enter the **Azure Tenant ID** and **Azure Subscription IDs**, then click **Test Connection**; on success the panel shows **Cloud accounts connected:**.

**Grant tenant permissions.** Linking subscriptions grants Atlas read-only **Azure RBAC** — enough for Azure-native discovery (Azure OpenAI, Azure Machine Learning, storage, AI Search) and Azure Databricks *workspace enumeration*. **SharePoint** and **Copilot Studio** discovery additionally require a one-time **tenant admin consent**, and Copilot Studio also needs a **Dataverse Application User**. Grant these after linking — see [Grant Atlas access to your tenant](/_docs/docs/providers/azure/tenant_setup).
...
- **Discovery** — from a linked Azure subscription, Atlas discovers your Copilot Studio agents, their components, tools, connectors, and authentication configurations across all Power Platform environments in the tenant. Enabling discovery requires **tenant admin consent** and a **Dataverse Application User** — see [Grant Atlas access to your tenant](/_docs/docs/providers/azure/tenant_setup). For what is discovered, see [MS Copilot Studio](/_docs/docs/providers/copilot_studio#agent-discovery).
...
[PreviousAWS Permission and Inventory Coverage](/_docs/docs/providers/aws/permission_breakdown)[NextTenant setup](/_docs/docs/providers/azure/tenant_setup)- [How Atlas integrates with an Azure AI environment](#how-atlas-integrates-with-an-azure-ai-environment)- [Link your Azure subscriptions for discovery](#link-your-azure-subscriptions-for-discovery)- [Discover Azure Databricks workspaces](#discover-azure-databricks-workspaces)- [How Atlas discovers SharePoint](#how-atlas-discovers-sharepoint)- [Use Azure OpenAI with AI Runtime and AI Red Team](#use-azure-openai-with-ai-runtime-and-ai-red-team)- [MS Copilot Studio: discovery and runtime](#ms-copilot-studio-discovery-and-runtime)- [Enable Copilot Studio connection identity (optional)](#enable-copilot-studio-connection-identity-optional)

...
url: https://prod.alltrue-be.com/_docs/docs/platform_services/313
...
Added support for multiple data plane installations to optimize gateway request handling by region and meet data residency requirements. Each data plane operates with its own proxy and OpenSearch instance. Officially supported regions now include eu-central-1, us-east-1, and us-west-2, with additional regions available upon request.
...
- Management: Register additional data planes, designate the control plane DEK holder, configure the OpenSearch logs sink, and select the data plane used for gateway-routed LLM pentests.
...
Removed the Threat Detection tab from AI Observability. OpenSearch-based alerting has been replaced with native alerting in AI Monitor, including drill-down and investigation capabilities. Native anomaly detection will be introduced in an upcoming release.

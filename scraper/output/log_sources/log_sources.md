...
The benefit is coverage and a single pane of glass: AI usage that never passes through the Atlas gateway — Claude usage pulled from Anthropic, Microsoft 365 Copilot or ChatGPT Enterprise activity from the Varonis Data Security Platform, browser-based AI captured by Island Browser, Claude Code and Claude Cowork telemetry, or AWS Bedrock model invocations — still shows up in your investigation view and can raise policy violations.
...
Log Sources currently require an **AWS data plane**; Azure data planes are not yet supported. Each per-source page repeats this requirement as a one-line note and links back here.

...
6. End with this exact check question: "A customer's AI agent has access to their internal file system, corporate email, and CRM database. Their security architect asks: what are the three main ways an attacker would try to abuse that agent, and which specific Atlas guardrails catch each one?"
...
7. End with this exact check question: "A customer uses Claude claude.ai Business across their organization, but they cannot route all Claude traffic through the Atlas gateway. Can Atlas still see and evaluate that Claude activity? How does it work, and what's the one important limitation?"
...
6. Explain how Agent Manifest fits into the broader Atlas governance workflow: Agent Manifest provides the baseline. AI SPM policies (like those introduced in v3.6.0) evaluate posture against governance requirements. Require Approval can hold sensitive actions for human review. Together these three form Atlas's agent governance stack: define the contract (Manifest) → monitor posture (AI SPM) → gate sensitive actions (Require Approval).
7. End with this exact check question: "A security architect asks: 'We already document what our AI agents are supposed to do in a wiki. How is Agent Manifest different, and how does Atlas actually use it to catch problems?' Give a complete answer and explain how Agent Manifest differs from IBAC."

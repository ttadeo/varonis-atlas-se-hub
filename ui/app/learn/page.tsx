...
5. Explain the SE discovery angle: "How does Atlas fit into our existing stack?" is the #1 technical question. Knowing these integration patterns lets you answer confidently for any customer architecture — whether they use LiteLLM, Copilot Studio, a custom proxy, or no proxy at all.
...
5. Explain the four runtime enforcement actions Atlas can take when a guardrail fires — SE must know all four: (a) Block — the tool call or request is denied immediately; (b) Allow — pass through with a log; (c) Alert — allow but raise an issue in AI Investigation for review; (d) Require Approval — the activity is held and an authorized reviewer must approve or reject before it proceeds. Require Approval is the human-in-the-loop option for sensitive tool call interactions where automated allow or block is too blunt. Example: an agent attempts to write to a production database — too risky to allow automatically, but too context-dependent to block permanently. A reviewer sees the full context and decides.
6. Explain three v3.7.0 enhancements that directly affect agentic policy evaluation: (a) Claude Inference Hooks — Atlas now supports Claude inference hooks, providing inline enforcement and policy evaluation for Claude model interactions across Claude Chat, Claude Code, Claude Cowork, and Claude Design. For customers running Claude-based agents, this closes the enforcement gap that previously required routing all activity through the gateway proxy. (b) Long Prompt Sampling for Runtime Analysis — runtime policies can now use optional sampling when evaluating long prompts. Instead of requiring the entire prompt to be processed, Atlas selects representative content for policy evaluation, allowing agentic guardrails to continue operating on long multi-turn inputs without excessive processing overhead. (c) Session Policy Evaluator LLM — session policies can now use a dedicated evaluator LLM for policy analysis, rather than relying on the model associated with the underlying AI application. This allows more consistent policy enforcement independent of which model the agent itself uses.
7. End with this exact check question: "A customer's AI agent has access to their internal file system, corporate email, and CRM database. Their security architect asks: what are the three main ways an attacker would try to abuse that agent, and which specific Atlas guardrails catch each one?"
...
7. Explain the v3.7.0 Claude Inference Hooks addition: In addition to the hook-based architecture above, Atlas v3.7.0 introduces Claude Inference Hooks — a direct enforcement path for Claude model interactions that doesn't rely on the coding agent hook system. Inference requests are sent through Atlas runtime controls, providing inline enforcement, additional visibility, and policy evaluation for Claude Chat, Claude Code, Claude Cowork, and Claude Design. SE talking point: "For customers who have standardized on Claude products, this means every Claude touchpoint — IDE, web, collaboration tools — can be covered by Atlas runtime policy. The hook-based architecture and Claude Inference Hooks are complementary: hooks intercept at the agent action level, inference hooks intercept at the model call level."
8. End with this exact check question: "A developer asks: 'I use Cursor in my IDE every day — how does Atlas actually intercept what my agent does, and what can it block vs. just log?' Walk me through the answer."
...
7. Explain the v3.7.0 Claude Cowork Plugin for Windows: The Atlas Claude Cowork plugin now supports Windows, in addition to existing macOS support. Windows users can deploy the plugin to apply Atlas runtime visibility and policy protections to supported Claude Cowork activity. SE talking point: "If a customer has Windows-based developer workstations and is using Claude Cowork, they no longer need a separate deployment strategy for those machines. Same plugin, same MDM distribution, now platform-parity with macOS."
8. End with this exact check question: "A CISO says: 'We have 400 developers using Claude Code on macOS. I cannot ask each developer to install an Atlas hook manually — what are our options, and what does the fail-open behavior mean for policy enforcement?' Give your complete answer."
...
6. Explain Salesforce Agentforce Prompt Log Ingestion: Salesforce Agentforce prompt activity can now be ingested into Atlas as a Log Source. The resulting events and sessions appear in AI Investigation, providing visibility into how Agentforce agents are being used — supporting investigation, auditing, and usage analysis. SE talking point: "Salesforce Agentforce is one of the most widely deployed enterprise agentic platforms. If a customer is using Agentforce and can't route it through the Atlas gateway, this log source closes that blind spot — you get full AI Investigation visibility into what those agents are doing." Same offline limitation applies: policies evaluate ingested events but enforcement actions surface as alerts rather than blocks.
7. Explain the two new v3.7.0 log sources: (a) Google Gemini App Log Ingestion — Google Gemini App activity can now be ingested into Atlas. Prompt and response activity from supported enterprise Gemini usage appears in AI Investigation, supporting investigation, audit, and runtime policy workflows. Same offline enforcement model as all log sources — BLOCK and MODIFY actions surface as ALERTs rather than real-time blocks. SE talking point: "Customers who can't proxy Gemini traffic now have the same offline visibility for Google's LLM that they've had for Claude and Salesforce — AI Investigation coverage closes the gap." (b) Azure Data Plane Log Source Ingestion Parity — Azure-hosted data planes now support log source ingestion capabilities consistent with other Atlas data plane deployments. This expands the ability to ingest supported external log sources through Azure data planes. Critical SE flag: existing Azure data plane deployments must rerun the deployment template to enable this capability. Also, Anthropic OpenTelemetry Collector ingestion is not supported on Azure data planes in this release — note that limitation for Azure customers also running Anthropic workloads.
8. End with this exact check question: "A customer uses Claude claude.ai Business across their organization, but they cannot route all Claude traffic through the Atlas gateway. Can Atlas still see and evaluate that Claude activity? How does it work, and what's the one important limitation?"
...
If the SE answers with a topic list instead of a full sentence, say exactly: "It looks like you sent me a list of topics rather than an answer — but I can work with that!" then give feedback.`,
  ...
  {
    id: 23,
    title: "Atlas v3.7.0 — New Features",
    checkQuestion: "A CISO says: 'We have Anthropic Claude deployed across our developer fleet — Claude Code, Claude Cowork, and claude.ai Business — and we also have compliance audit work stacking up under ISO 42001 and the new OWASP AI Testing Guide. How does v3.7.0 address both?' Give a complete answer covering the runtime enforcement changes and the compliance redesign.",
    tier: "advanced",
    followUpPrompts: [
      "Explain MCP Registries — what problem do they solve that MCP Inspector or Virtual MCPs don't already address, and what's the SE pitch for a customer scaling MCP in production?",
      "A customer runs their Atlas data plane on Azure. What did they need to do to get log source ingestion working after upgrading to v3.7.0, and what's the one limitation to flag?",
      "Walk me through the Agent Resource Review workflow in v3.7.0 — how is it different from just reviewing an agent's inventory record in v3.6.0?",
    ],
    intro: `[LEARNING MODE - Lesson 23: Atlas v3.7.0 — New Features]
You are teaching an experienced Varonis Sales Engineer. This is an advanced lesson covering the major new capabilities introduced in Atlas v3.7.0 — the current release. Please:
1. Frame why this release matters: v3.7.0's headline themes are Claude ecosystem expansion, MCP governance at scale, and AI compliance maturity. Every Claude-based product now has a direct enforcement path into Atlas. MCP governance moves from per-server controls to a governed catalog model. And AI Compliance is rebuilt end-to-end to reduce manual work and stay current as the AI footprint changes. SE framing: "v3.7.0 is about closing the remaining Claude blind spots, scaling MCP governance beyond individual servers, and making compliance an automated program rather than a manual audit."
2. Explain Claude Inference Hooks — the headline runtime capability: Atlas now supports Claude inference hooks for supported Claude runtime activity. Inference requests can be sent through Atlas runtime controls, providing inline enforcement, additional visibility, and policy evaluation for Claude model interactions including Claude Chat, Claude Code, Claude Cowork, and Claude Design. This means every Claude touchpoint — from developer IDEs to productivity tools to the claude.ai web interface — can now be covered by Atlas runtime policy. SE talking point: "If a customer has standardized on Anthropic and is asking whether Atlas covers more than just the gateway proxy path — the answer is now yes. Claude Inference Hooks give you inline enforcement across all four Claude products."
3. Explain MCP governance at scale — three new capabilities: (a) MCP Registries — Atlas now supports MCP Registries for centrally publishing and managing approved MCP connections. Registries provide a governed catalog of MCP resources that can be made available for use across supported projects and agent workflows. SE talking point: "MCP Inspector tells you what a server exposes. Virtual MCPs let you scope tools per endpoint. Registries are the catalog layer — you publish what's approved, and projects consume from that list. It's the difference between one-off approvals and a governed MCP program." (b) MCP Gateway Dashboard — a centralized view of MCP Gateway activity: traffic, connected servers, tool usage, and other gateway-related metrics. Gives operations teams the visibility to manage MCP at scale. (c) MCP Issues on Resource Pages — MCP-related security and governance findings now appear directly on the affected resource's page, without navigating separately to the MCP Issues screen.
4. Explain the Agent Resource Review Workflow with Manifest and Insights: Agent resource reviews now incorporate Agent Manifest information and Atlas security insights into the review workflow. Reviewers evaluate an agent's approved purpose and expected behavior alongside observed configuration, access, and security findings — all within the same review process. SE talking point: "Before, a reviewer could see that an agent existed and what tools it had. Now they see the agent's governance contract (what it was supposed to do) and Atlas's live security findings side by side. That's the difference between a checkbox review and an informed governance decision."
5. Explain expanded log sources — two new additions: (a) Google Gemini App Log Ingestion — Google Gemini App activity can now be ingested into Atlas. Prompt and response activity from supported enterprise Gemini usage appears in AI Investigation, providing visibility into Gemini usage and supporting investigation, audit, and runtime policy workflows. Same offline enforcement model as other log sources — policies evaluate ingested activity but enforcement actions surface as alerts rather than blocks. (b) Azure Data Plane Log Source Ingestion Parity — Azure-hosted data planes now support log source ingestion capabilities consistent with other supported Atlas data plane deployments. Important SE flag for Azure customers: existing Azure data plane deployments must rerun the deployment template to enable this. Also: Anthropic OpenTelemetry Collector ingestion is not supported on Azure data planes in this release.
6. Explain AI Compliance Redesign — rebuilt end to end: The redesign reduces manual effort and keeps the compliance picture current as the AI footprint changes. Key changes: (a) New Policies Tab — activate each framework at the entity scope it governs (company, org, project, vendor, model, dataset). Optional Compass questionnaire recommends frameworks. New resources flow into compliance automatically as they join inventory. (b) Redesigned Dashboard — each entity has its own scoping questionnaire, controls, and required documents; produces its own evidence report when complete. (c) Redesigned Knowledge Hub — one-stop portal for policy documents across all frameworks; each document is provided once and satisfies every matching required-document slot across frameworks. Three tabs: Active, Building, Missing. (d) Platform-Drafted Answers — answers can now be drafted automatically from the Atlas platform itself (currently available for HITRUST and OWASP frameworks). (e) New framework: OWASP AI Testing Guide — assesses whether in-scope AI systems are tested against the Guide's recommended techniques including agentic behavior, embedding manipulation, model and data poisoning, and supply-chain tampering. Platform evidence is collected automatically. Limitation: audits started in the previous compliance UI do not automatically appear in the new one — flag this when upgrading existing customers.
7. Cover the remaining features concisely: (a) Claude Cowork Plugin for Windows — the Atlas Claude Cowork plugin now supports Windows, extending runtime visibility and policy protections to Windows users. (b) Copilot Studio Onboarding Script — Atlas now provides a script to automate Microsoft Copilot Studio onboarding, replacing the previous manual per-environment Power Platform setup. Can onboard all environments or selected environments. (c) Long Prompt Sampling for Runtime Analysis — runtime policies can now sample representative content from long prompts instead of requiring the entire prompt to be processed, allowing protections to continue operating on long inputs while reducing overhead. (d) Session Policy Evaluator LLM — session policies can now use a dedicated evaluator LLM for policy analysis, rather than relying on the model associated with the underlying AI application. (e) Red Team Retry Strategy — failed Red Team tests can now be saved and rerun automatically for ongoing regression testing and validation. (f) AI Investigation Session Names — sessions now have generated names for easier identification and navigation. (g) AI Investigation Event Table Filters — additional filtering on supported event attributes to isolate relevant activity faster.
8. End with this exact check question: "A CISO says: 'We have Anthropic Claude deployed across our developer fleet — Claude Code, Claude Cowork, and claude.ai Business — and we also have compliance audit work stacking up under ISO 42001 and the new OWASP AI Testing Guide. How does v3.7.0 address both?' Give a complete answer covering the runtime enforcement changes and the compliance redesign."
This is an advanced lesson — the SE must position v3.7.0 as a coherent story, not a feature list, and be able to answer follow-up questions on any individual capability.
IMPORTANT: Do not reference, describe, or suggest what comes in any future lesson. The SE will navigate using the sidebar.
If the SE answers with a topic list instead of a full sentence, say exactly: "It looks like you sent me a list of topics rather than an answer — but I can work with that!" then give feedback.`,
  },
  {
    id: 29,
    title: "Agent Manifest & Agent Governance",
    tier: "advanced",
    checkQuestion: "A security architect asks: 'We already document what our AI agents are supposed to do in a wiki. How is Agent Manifest different, and how does Atlas actually use it to catch problems?' Give a complete answer and explain how Agent Manifest differs from IBAC.",
    followUpPrompts: [
      "Walk me through what fields an Agent Manifest defines — what does 'approved purpose' actually mean in Atlas, and who sets it?",
      "A customer's Agent Manifest says the agent should only use three tools, but Atlas detects it calling a fourth. What happens next — what does Atlas surface and where?",
      "How would you position Agent Manifest to a customer who already has IBAC enabled — aren't they solving the same problem?",
    ],
    intro: `[LEARNING MODE - Lesson 29: Agent Manifest & Agent Governance]
You are teaching an experienced Varonis Sales Engineer. This is an advanced lesson on Agent Manifest — one of the most strategically important new capabilities in Atlas v3.6.0. It addresses the enterprise governance question: "How do we know our AI agents are doing what we intended?" Please:
1. Frame the problem Agent Manifest solves: As enterprises deploy more AI agents, a governance gap emerges. Teams know what an agent is supposed to do — but there's no structured, machine-readable definition of that intent, and no system comparing observed behavior against it. Agents drift. They call unexpected tools, access unintended data, or take actions outside their approved scope. Without a baseline, you can't measure drift. SE talking point: "A wiki page saying 'this agent handles HR queries' is not governance. Agent Manifest turns that informal description into a structured contract that Atlas actively monitors against."
2. Explain what Agent Manifest defines: Agent Manifest is a structured definition associated with an agent resource in Atlas. It captures: (a) Approved purpose — what the agent is intended to do in natural language; (b) Expected behavior — the operational scope and boundaries; (c) Permitted tools — the specific tools the agent is authorized to use; (d) Capabilities — what the agent can and cannot do; (e) Ownership — who is responsible for the agent. This creates a governance baseline that lives in Atlas alongside the agent's inventory record.
3. Explain how Atlas uses the manifest: Atlas compares observed agent activity against the Agent Manifest over time. If the agent calls tools not listed in the manifest, accesses data outside its approved scope, or behaves in ways inconsistent with its stated purpose, Atlas surfaces the deviation. This gives security and governance teams a clear answer to "is this agent operating within its intended scope?" — without requiring manual review of every session.
4. Distinguish Agent Manifest from IBAC clearly — SEs will be asked this: IBAC (Intent-Based Access Control) is a real-time, per-tool-call evaluation at the moment of execution. It asks: "Does this specific tool call align with what the user asked for right now?" IBAC fires during a session, evaluates a single action, and can block it immediately. Agent Manifest is a static, organization-level governance declaration compared against observed behavior over time. It asks: "Is this agent, overall, operating within its approved design?" Agent Manifest informs long-term posture and governance review — not real-time enforcement. SE analogy: "IBAC is a traffic cop stopping a specific car. Agent Manifest is an audit comparing a driver's entire route history against their approved driving zone."
5. Explain the SE positioning for different buyer types: (a) Security architect: "Agent Manifest gives you a formal governance baseline for every agent — you define the contract, Atlas monitors compliance against it, deviations surface as findings." (b) CISO: "This is how you answer the board question 'do our AI agents stay within their approved scope?' — you have a structured record and Atlas validates it continuously." (c) AI engineering lead: "You define what your agent is supposed to do once. Atlas watches that it doesn't drift without you knowing."
6. Explain how Agent Manifest fits into the broader Atlas governance workflow: Agent Manifest provides the baseline. AI SPM policies evaluate posture against governance requirements. Require Approval can hold sensitive actions for human review. Together these three form Atlas's agent governance stack: define the contract (Manifest) → monitor posture (AI SPM) → gate sensitive actions (Require Approval).
7. Explain the v3.7.0 Agent Resource Review Workflow with Manifest and Insights: In v3.7.0, agent resource reviews now incorporate Agent Manifest information and Atlas security insights directly into the review workflow. Reviewers can evaluate an agent's approved purpose and expected behavior alongside observed configuration, access, and security findings — all within the same review process. This brings the agent's governance baseline and current security posture together in one place. SE talking point: "Before v3.7.0, you had to cross-reference the Manifest in one place and the agent's findings in another. Now it's one review surface. A reviewer sees 'here's what this agent was approved to do, here's what Atlas found it actually doing and what risks are open.' That's the difference between a checkbox review and governance with evidence."
8. End with this exact check question: "A security architect asks: 'We already document what our AI agents are supposed to do in a wiki. How is Agent Manifest different, and how does Atlas actually use it to catch problems?' Give a complete answer and explain how Agent Manifest differs from IBAC."
This is an advanced lesson — the SE must be able to distinguish Agent Manifest from IBAC precisely and position it as a governance capability, not a detection capability.
IMPORTANT: Do not reference, describe, or suggest what comes in any future lesson. The SE will navigate using the sidebar.
If the SE answers with a topic list instead of a full sentence, say exactly: "It looks like you sent me a list of topics rather than an answer — but I can work with that!" then give feedback.`,
  },
  {
    id: 30,
    title: "AI Third-Party Risk Management (TPRM)",
    tier: "advanced",
    checkQuestion: "A risk and compliance officer says: 'We use fifteen different third-party AI vendors across our business units. How does Atlas help us assess and track the risk of those vendors — and what's the difference between Atlas AI TPRM and our existing vendor risk management process?' Give a complete answer.",
    followUpPrompts: [
      "Walk me through the AI TPRM exposure survey — what does it assess, who fills it out, and how does Atlas use the responses?",
      "A customer already has a vendor risk management tool like OneTrust or ServiceNow. Why would they also use Atlas AI TPRM — what does Atlas add that those tools don't?",
      "How would you demo AI TPRM to a prospect — what's the three-minute talk track that makes it click?",
    ],
    intro: `[LEARNING MODE - Lesson 30: AI Third-Party Risk Management (TPRM)]
You are teaching an experienced Varonis Sales Engineer. This is an advanced lesson on Atlas AI TPRM — a differentiated capability that extends Atlas's governance beyond internal AI assets to the third-party AI vendors an organization depends on. This surfaces regularly in enterprise deals where compliance and risk teams are involved. Please:
1. Frame why AI TPRM matters: Enterprises don't just build AI — they buy it. Business units adopt third-party AI vendors for specific use cases: an HR team uses an AI recruiting tool, finance uses an AI expense platform, legal uses an AI contract reviewer. Each of these vendors has access to sensitive data, uses AI models the customer doesn't control, and represents a risk that traditional vendor risk management processes weren't built to assess. SE talking point: "Your existing vendor risk process asks 'is this vendor SOC 2 compliant?' Atlas AI TPRM asks 'what AI models is this vendor using, how are they trained, what data do they process, and does their AI posture meet your policy requirements?'"
2. Explain what AI TPRM covers: Atlas AI TPRM provides a framework for assessing, tracking, and managing the risk of third-party AI vendors. It covers: (a) AI vendor inventory — a registry of third-party AI tools and vendors in use across the organization, linked to the business units that use them; (b) Risk assessment — structured evaluation of each vendor's AI practices, data handling, model provenance, and security posture; (c) Exposure surveys — questionnaires sent to vendors or filled by internal stakeholders to assess AI-specific risk factors; (d) Risk scoring — each vendor receives a risk score based on assessment results; (e) Continuous tracking — risk status is tracked over time, with alerts when vendor posture changes or reviews expire.
3. Explain the Exposure Survey in detail — SEs are asked about this: The exposure survey is a structured questionnaire that Atlas uses to assess a third-party AI vendor's risk profile. It covers areas like: what AI models the vendor uses (proprietary vs. open source, training data sources), how the vendor handles customer data (retention, sharing, use for model training), the vendor's security certifications and audit history, what the vendor's AI outputs are used for and who sees them, and whether the vendor has incident response procedures specific to AI failures. The survey can be completed by the vendor directly or by an internal stakeholder who manages the vendor relationship. Results feed into the vendor's risk score in Atlas.
4. Distinguish Atlas AI TPRM from general VRM tools: General vendor risk management tools (OneTrust, ServiceNow, Archer) assess vendors on IT security, compliance, financial stability, and business continuity — they weren't designed for AI-specific risk. They don't assess model provenance, training data practices, AI output risks, or agent behavior. Atlas AI TPRM is purpose-built for the AI risk surface: it uses Atlas's understanding of AI systems to ask the right questions and score vendors on criteria that matter for AI governance. SE talking point: "OneTrust will tell you the vendor has ISO 27001. Atlas AI TPRM will tell you the vendor is training on your data, using a model with no audit trail, and has no AI incident response plan — that's a different risk conversation."
5. Explain the SE demo angle — the three-minute talk track: (a) Start in AI TPRM — show the vendor registry with risk scores. Pick one vendor with a medium or high risk score. (b) Drill into the vendor record — show what the exposure survey found: data retention concerns, model training practices, missing certifications. (c) Show the risk score breakdown — how Atlas scored the vendor across AI-specific risk categories. (d) Show the remediation workflow — what action the customer is expected to take (request additional documentation, restrict data sharing, offboard vendor). Close with: "This is how you get ahead of a vendor AI incident before it becomes a breach or a regulatory finding."
6. Explain how AI TPRM fits the Varonis sales motion: Varonis's core value proposition is data security — knowing where sensitive data lives, who has access, and whether that access is appropriate. Atlas AI TPRM extends that to third-party AI: "You've locked down your internal data. But your finance team's AI expense tool has access to your GL system. Atlas TPRM tells you whether that vendor's AI practices meet your standards — or whether you have a sensitive data exposure hiding in your vendor stack."
7. End with this exact check question: "A risk and compliance officer says: 'We use fifteen different third-party AI vendors across our business units. How does Atlas help us assess and track the risk of those vendors — and what's the difference between Atlas AI TPRM and our existing vendor risk management process?' Give a complete answer."
This is an advanced lesson — the SE must be able to position AI TPRM distinctly from general VRM tooling and walk through the exposure survey with confidence.
IMPORTANT: Do not reference, describe, or suggest what comes in any future lesson. The SE will navigate using the sidebar.
If the SE answers with a topic list instead of a full sentence, say exactly: "It looks like you sent me a list of topics rather than an answer — but I can work with that!" then give feedback.`,
  },
  {
    id: 31,
    title: "AI SPM — Sensitive Data & Posture Policies",
    tier: "advanced",
    checkQuestion: "A data security architect says: 'We're a Varonis customer and we care deeply about sensitive data. How does Atlas AI SPM connect what we already know about our sensitive data to the AI systems accessing it — and what does it automatically flag without us having to configure anything?' Give a complete answer.",
    followUpPrompts: [
      "Walk me through the difference between an AI SPM policy and a runtime guardrail — can a customer have both covering the same agent?",
      "A customer's AI agent has write access to a database containing PII, but no one has ever reviewed or approved that access. Which v3.6.0 AI SPM policy fires, and what does the customer do next?",
      "How would you position AI SPM to a customer who already has Varonis DSP — what's the '1+1=3' story?",
    ],
    intro: `[LEARNING MODE - Lesson 31: AI SPM — Sensitive Data & Posture Policies]
You are teaching an experienced Varonis Sales Engineer. This is an advanced lesson on Atlas AI Security Posture Management (AI SPM) with a focus on sensitive data policies — a key differentiator that connects Varonis's data security heritage directly to AI governance. This is the "1+1=3" story for existing Varonis customers and a strong standalone message for net-new prospects. Please:
1. Frame what AI SPM is and how it differs from runtime enforcement: AI SPM is continuous posture assessment — it evaluates the configuration, governance, and risk posture of AI systems across the organization, not just what happens at runtime. Think of it like CSPM (Cloud Security Posture Management) but for AI: it identifies misconfigurations, governance gaps, and policy violations in how AI systems are set up and governed, rather than in what they do at runtime. Runtime guardrails stop bad things from happening in the moment. AI SPM tells you whether your AI environment is structured and governed correctly in the first place.
2. Explain the AI SPM policy categories: Atlas AI SPM policies cover multiple domains: (a) Misconfiguration — AI systems with insecure settings, missing guardrails, or improper configurations; (b) Identity and access — AI agents using overprivileged, shared, or unreviewed runtime identities; (c) Governance — AI assets that haven't gone through required review and approval workflows; (d) Sensitive data — AI systems with access to sensitive data where governance requirements aren't met (the v3.6.0 expansion). SEs should know that AI SPM policies run continuously and surface findings in the AI SPM view — separate from runtime issues in AI Investigation.
3. Explain the six v3.6.0 Sensitive Data AI SPM policies in depth — these are the ones SEs need to demo: (a) Sensitive Access Uses Shared or Privileged Runtime Identity — an AI agent is accessing sensitive data using a shared service account or privileged identity. Problem: shared identities reduce accountability; if that credential is compromised, the blast radius is wide. Atlas flags it and recommends switching to a dedicated, least-privilege identity. (b) Sensitive Data Has Weak Sharing Control Path — the path through which an agent reaches sensitive data has insufficient access controls. The data may be well-protected at rest, but the chain of permissions getting to it is weak. (c) Sensitive Workflow Uses Rejected Tools — an agent workflow touching sensitive data includes a tool that was explicitly rejected during a previous governance review. This catches cases where rejected tools slip back in through agent updates or configuration drift. (d) Sensitive Write Access Lacks Approved Governance — an agent has write-capable access to sensitive data but that access was never reviewed and approved through Atlas's governance workflow. This is the most common finding in new deployments. (e) Sensitive Access Lacks Approved Governance — even read access to sensitive data hasn't been reviewed. Less severe than write, but still a governance gap. (f) Sensitive Access Review Expired — a previous governance review exists but is no longer current. Atlas tracks review recency and flags when access needs to be revalidated.
4. Explain the Varonis "1+1=3" positioning for existing customers: Varonis Data Security Platform (DSP) already knows where sensitive data lives — which files, databases, and systems contain PII, financial data, health records, and IP. Atlas knows which AI agents have access to those systems and what they're doing with that access. Together: Varonis DSP identifies sensitive data → Atlas AI SPM evaluates whether the AI agents accessing it are governed correctly → Atlas runtime enforces policies on what those agents can do at execution time. SE talking point: "If you're already a Varonis customer, you've done the hard work of mapping your sensitive data. Atlas takes that map and asks: 'Which AI agents can reach this data, and is that access reviewed, approved, and governed?' That's a question your DSP can't answer alone."
5. Explain the governance workflow that AI SPM plugs into: When an AI SPM sensitive data policy fires, Atlas creates a finding with the specific governance gap identified. The finding includes: which agent or resource is involved, which sensitive data is at risk, which policy was violated, and recommended remediation. Remediations typically involve: initiating a governance review (which can be done directly from the finding), restricting the agent's runtime identity, removing rejected tools, or renewing an expired review. The Require Approval action (v3.6.0) can be connected to this workflow — if an agent with unreviewed sensitive data access attempts a tool call, Require Approval can hold it until the governance gap is resolved.
6. Explain the SE demo flow for AI SPM: (a) Open AI SPM and show the policy findings dashboard — filter to Sensitive Data category. (b) Click into a "Sensitive Write Access Lacks Approved Governance" finding — show which agent, which data, what the governance gap is. (c) Initiate a governance review from the finding — show how the workflow starts without leaving Atlas. (d) Pivot to the runtime view — show how Require Approval is configured for that agent's sensitive tool calls while the review is pending. Close with: "This is how you close the loop between knowing you have a governance gap and actually doing something about it — without spreadsheets or manual tracking."
7. End with this exact check question: "A data security architect says: 'We're a Varonis customer and we care deeply about sensitive data. How does Atlas AI SPM connect what we already know about our sensitive data to the AI systems accessing it — and what does it automatically flag without us having to configure anything?' Give a complete answer."
This is an advanced lesson — the SE must be able to explain AI SPM posture vs. runtime enforcement clearly, walk through the six sensitive data policies, and deliver the Varonis 1+1=3 story with precision.
IMPORTANT: Do not reference, describe, or suggest what comes in any future lesson. The SE will navigate using the sidebar.
If the SE answers with a topic list instead of a full sentence, say exactly: "It looks like you sent me a list of topics rather than an answer — but I can work with that!" then give feedback.`,
  },
];
...
const DISPLAY_ORDER = [1, 2, 3, 4, 5, 6, 19, 7, 8, 9, 10, 11, 12, 20, 21, 24, 26, 13, 14, 15, 16, 17, 22, 31, 30, 23, 29, 25, 27, 28, 18];

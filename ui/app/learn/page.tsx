...
If the SE answers with a topic list instead of a full sentence, say exactly: "It looks like you sent me a list of topics rather than an answer — but I can work with that!" then give feedback.`,
  ...
  {
    id: 23,
    title: "Atlas v3.5.0 — New Features",
    checkQuestion: "A customer's security architect says: 'Our Claude Code sessions are calling tools we never intended to approve. How does Atlas catch that drift automatically, and can it show us exactly when and why it happened?' Walk me through v3.5.0's answer.",
    tier: "advanced",
    followUpPrompts: [
      "What's the difference between IBAC Intent Drift and the standard agentic guardrail rules — why do you need both?",
      "A customer uses Claude on claude.ai and Claude Code in the IDE — which two v3.5.0 features give Atlas visibility into both, and what's the difference between them?",
      "Walk me through the Atlas MCP Server — how does a developer actually use it and what can they ask it to do?",
    ],
    intro: `[LEARNING MODE - Lesson 23: Atlas v3.5.0 — New Features]
You are teaching an experienced Varonis Sales Engineer. This is an advanced lesson covering the major new capabilities introduced in Atlas v3.5.0 (released July 13, 2026) — the features customers and prospects will be asking about now. Please:
1. Frame why this release matters: v3.5.0 is Atlas's deepest investment in coding agent security to date. The headline theme is governance at the point of development — the risk surface has moved from chat applications into developers' IDEs and terminals, and v3.5.0 closes that gap across nine coding agents, adds intelligence beyond simple allow/deny, and lets coding agents themselves query Atlas as a security tool.
2. Explain IBAC Intent Drift Detection (the marquee new policy): Intent-Based Access Control (IBAC) evaluates whether a tool call aligns with the user's actual request or intended business task — not just whether the tool is permitted. If a developer asks Claude Code to "fix the login bug" and the agent then calls a shell tool to push to a production branch, that push may be technically allowed but is clearly outside the stated intent. IBAC catches this class of drift. Pair with the Intent Timeline: AI Investigation Sessions now display a timeline of how user intent, agent behavior, and tool activity evolved across the session — investigators can see exactly when drift began, what triggered the IBAC policy, and what happened before and after. SE talking point: "It's not enough to know a tool was blocked. IBAC tells you why — it showed the agent drifting from what the developer actually asked for."
3. Explain the Atlas MCP Server: Coding agents (Claude Code, Cursor, OpenAI Codex CLI, and other MCP clients) can now connect to an Atlas MCP Server endpoint and query Atlas directly from their IDE or terminal. Developers can ask their agent to retrieve AI inventory, review issues, check compliance status, inspect pentest results, or work with Virtual MCP configurations — without opening the Atlas UI. This is bidirectional: Atlas governs coding agents via hooks; coding agents can query Atlas via MCP Server. SE demo angle: "After showing how Atlas blocks a bad tool call, pivot to showing the developer asking their agent 'what does Atlas say about our posture right now?' — right inside their terminal."
4. Explain expanded coding agent coverage: v3.5.0 adds runtime coverage for five new agents — Devin CLI, Devin Desktop, Google Antigravity, Kiro CLI, and OpenAI Codex — bringing the total to nine supported agents. Also added: Claude Cowork Hooks (plugin-based enforcement for Claude Cowork). SE talking point: "Wherever developers are using AI to write code, Atlas now has an enforcement point. Nine agents, one policy plane."
5. Explain the two new log ingestion sources for Claude: (a) Claude OTEL Log Ingestion — Claude OpenTelemetry logs for Claude Cowork and Claude Code can be ingested into Atlas; activity is reviewed for investigation and visibility, and runtime policies apply to ingested events for alerting. Note: limited to a subset of event types based on current Claude support. (b) Copilot and ChatGPT Enterprise Prompt Sync with Varonis DSPM — prompt activity from Microsoft Copilot and ChatGPT Enterprise can now be correlated with Varonis DSPM context, improving sensitive data visibility and investigation workflows. Both provide offline evaluation (alerts only, no inline blocking).
6. Explain Malicious Skill Detection AI SPM Policies: v3.5.0 ships six new AI SPM policies specifically for detecting malicious or risky agent skills discovered in repositories. Key policies: Malicious Skill Exfiltration Pattern (detects skills containing data exfiltration patterns), Skill Contains Hardcoded Credential, Skill Allows Unrestricted Shell Execution, Skill Accesses Local Credentials in Executable Context, Skill Performs External Data Transfer in Executable Context, Skill Uses Dynamic Shell Context. SE talking point: "When AI Inventory scans your GitHub repos and finds a Claude Code agent with a skill that silently exfiltrates data — that's a supply chain attack. These six policies surface it automatically."
7. Mention three additional notable items: (a) Data Access Graph — new visualization on Agent resource pages showing exactly which data sources an agent can reach; surfaces sensitive data exposure paths at a glance; (b) Salesforce Agentforce Discovery — Atlas can now discover and inventory Salesforce Agentforce agents, extending coverage to one of the most widely deployed enterprise agentic platforms; (c) Multimodal Pentest Strategy — Red Team assessments now support image-based test inputs for multimodal prompt injection attacks.
8. End with this exact check question: "A customer's security architect says: 'Our Claude Code sessions are calling tools we never intended to approve. How does Atlas catch that drift automatically, and can it show us exactly when and why it happened?' Walk me through v3.5.0's answer."
This is an advanced lesson — the SE must be able to position v3.5.0 as a coherent story (not a feature list) and map specific capabilities to customer pain points.
IMPORTANT: Do not reference, describe, or suggest what comes in any future lesson. The SE will navigate using the sidebar.
If the SE answers with a topic list instead of a full sentence, say exactly: "It looks like you sent me a list of topics rather than an answer — but I can work with that!" then give feedback.`,
  },
];
